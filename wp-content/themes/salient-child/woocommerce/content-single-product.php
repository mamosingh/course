...
<style>
    ...
    .enquire_this_product i {
        padding-right: 4px;
        transition: .3s all ease-in-out;
    }

    .enquire_this_product:hover i {
        padding-right: 10px;
    }
</style>
    ...
    <?php } else { ?>
        ...
        <div class="summary entry-summary">
            ...
            foreach ($terms as $term) {
                ...
                //1391 - Furniture
                //1392 - Lighting
                if ($productCategoryId == 1391 || $productCategoryId == 1392) {
                    $currentUri = 'www.' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
                    $productName = $product -> get_name();
                    $productName = strip_tags($productName);
//                    $productName = mb_strtolower($productName);
//                    $productName = str_replace(' ', '-', $productName);
                    $enquireButton =  (
                        '<a class="enquire_this_product" href="/enquire?productName='.$productName.'&link='.$currentUri.'">'.
            ...
            $args = array($enquireButton);
            add_action('woocommerce_after_add_to_cart_button', function() use ($args) {
                echo $args[0];
            }, 30);
